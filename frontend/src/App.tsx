...
import { FactChecksPanel } from './components/FactChecks'
    ...
    function updateFromProgress(evt: ProgressEvent) {
        ...
        setStageState((prev) => {
            ...
            if (stage === 'pipeline' && status === 'error') {
                // mark current running state as error
            }
            return next
        })
    }
                        ...
                        <section className="border rounded bg-white p-4 space-y-3">
                            <div className="flex flex-col md:flex-row gap-2">
                                <input 
                                    ...
                                    onKeyDown={(e) => {if (e.key === 'Enter') run().catch(() => {})}} 
                                    placeholder="Ask a research question..."
                                    className="flex-1 border rounded px-3 py-2 text-sm"
                                />
                                <button onClick={() => run()} disabled={loading} className="px-4 py-2 rounded bg-gray-900 text-white text-sm disabled:opacity-50">
                        ...
                        {current?.needs_clarification ? (
                            <section className="border rounded bg-white p-4">
                                ...
                                <ul className="list-disc pl-6 mt-2 text-sm">
                                    {current.clarifying_question.map((q, i) => (
                                        <li key={i}>{q}</li>
                                    ))}
                                </ul>
                            </section>
