...
async def list_sessions(limit: int = Query(default=50, ge=1, le=200), q: str | None = Query(default=None, max_length=200), tag: str | None = Query(default=None, max_length=50), db: AsyncSession = Depends(get_db)):
    sessions = await crud.list_sessions(db, limit=limit, q=q, tag=tag)
...
async def update_session(session_id: uuid.UUID, req: SessionUpdateRequest, db: AsyncSession = Depends(get_db)):
    s = await crud.get_session(db, session_id)
    if not s:
        raise HTTPException(status_code=404, detail='Session not found')

    await crud.update_session_meta(db, session_id, title=req.title, pinned=req.pinned, tags=req.tags)

    s2 = await crud.get_session(db, session_id)
    steps = await crud.get_session_steps(db, session_id)
    sources = await crud.get_session_sources(db, session_id)
    checks = await crud.get_session_fact_checks(db, session_id)
    summary_md = None
    ...
    for st in steps:
        if st.agent_name.startswith('summarizer') and st.output and isinstance(st.output, dict):
            if 'answer_markdown' in st.output:
                summary_md = st.output.get('answer_markdown')
                break

    return SessionDetail(
        id = s2.id,
        user_query = s2.user_query,
        title = getattr(s2, 'title', None),
        tags = getattr(s2, 'tags', []) or [],
        pinned = getattr(s2, 'pinned', False),
        status = s2.status,
        error = s2.error,
