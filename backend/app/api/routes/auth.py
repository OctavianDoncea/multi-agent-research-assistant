...
from app.schemas import AuthCredentials, AuthSessionOut, UserOut
...
async def _issue_session(db: AsyncSession, response: Response, user: User) -> AuthSessionOut:
    ...
    return AuthSessionOut(id=user.id, email=user.email, access_token=token)
...
async def logout(
    ...
    raw = raw or session_token
    if raw:
        await crud.revoke_auth_session_by_token_hash(db, hash_session_token(raw))
